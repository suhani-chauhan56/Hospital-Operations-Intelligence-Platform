...
SELECT
    ...
    DENSE_RANK() OVER (
        ORDER BY COALESCE(SUM(b.paid_amount), 0) DESC
    ) AS revenue_rank
FROM
    departments d
    ...
    census AS (
        SELECT c.census_date, (
                ...
                WHERE
                    a.admit_date <= c.census_date
                    AND COALESCE(
                        a.discharge_date, c.census_date
                    ) >= c.census_date
            ) AS occupied_beds
        FROM calendar c
    )
SELECT
    ...
    LEAST(
        100,
        occupied_beds / 500 * 100
    ) AS occupancy_pct,
    AVG(occupied_beds) OVER (
        ORDER BY
            census_date ROWS BETWEEN 6 PRECEDING
            AND CURRENT ROW
...
SELECT p.patient_id, p.age, p.gender, a.admission_id, a.admit_date, a.discharge_date, d.department, doc.doctor_id, dg.disease_severity_score, l.lab_abnormality_score, m.medicine_code, m.medicine_count, b.billed_amount, b.paid_amount, c.claim_status
FROM
    patients p
