...
CREATE TABLE admissions (
    ...
    CONSTRAINT fk_admission_doctor FOREIGN KEY (doctor_id) REFERENCES doctors (doctor_id),
    CONSTRAINT fk_admission_department FOREIGN KEY (department_id) REFERENCES departments (department_id),
    CHECK (
...
CREATE TABLE procedures (
    ...
    CONSTRAINT fk_procedure_admission FOREIGN KEY (admission_id) REFERENCES admissions (admission_id)
);
...
CREATE TABLE medicines (
    ...
    CONSTRAINT fk_medicine_admission FOREIGN KEY (admission_id) REFERENCES admissions (admission_id)
);
...
CREATE TABLE billing (
    ...
    CONSTRAINT fk_billing_admission FOREIGN KEY (admission_id) REFERENCES admissions (admission_id),
    CONSTRAINT fk_billing_patient FOREIGN KEY (patient_id) REFERENCES patients (patient_id),
    CONSTRAINT fk_billing_insurance FOREIGN KEY (insurance_id) REFERENCES insurance (insurance_id)
);
...
CREATE TABLE claims (
    ...
    CONSTRAINT fk_claim_billing FOREIGN KEY (billing_id) REFERENCES billing (billing_id),
    CONSTRAINT fk_claim_insurance FOREIGN KEY (insurance_id) REFERENCES insurance (insurance_id)
);
...
CREATE TABLE appointments (
    ...
    scheduled_time TIME,
    CONSTRAINT fk_appointment_admission FOREIGN KEY (admission_id) REFERENCES admissions (admission_id),
...
FROM admissions
WHERE
    Key_name = 'idx_admissions_patient_date';
...
CREATE INDEX idx_billing_patient_date ON billing (
    patient_id,
    claim_billing_date
);
...
CREATE INDEX idx_efficiency_scope_score ON hospital_efficiency_scores (scope_type, efficiency_score);

CREATE INDEX idx_recommendations_priority ON operational_recommendations (priority);